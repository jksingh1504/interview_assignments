...
router.post("/add", async (req, res, next) => {
    ...
    try {
        ...
        if (!productId) {
            throw new error_1.CustomError(400, "product id not provided");
        }
        ...
        if (existingProduct) {
            throw new error_1.CustomError(400, "Product already exists in the wishlist");
        }
        else {
...
router.delete("/remove", async (req, res, next) => {
    ...
    */
    try {
        ...
        let wishlist = await wishlist_1.default.findOne({ user: userId });
        if (!wishlist) {
            throw new error_1.CustomError(404, "wishlist not found");
        }
        const removedwishlist = wishlist.products.find((item) => item.product.toString() === productId);
...
router.get("/", async (req, res, next) => {
    ...
    try {
        const { userId } = req;
        let wishlist = await wishlist_1.default.findOne({ user: userId }).populate("products.product");
        if (!wishlist) {
            throw new error_1.CustomError(404, "wishlist not found");
        }
